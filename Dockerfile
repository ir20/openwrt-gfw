...
SHELL ["/bin/bash", "-c"]
RUN apt-get update
RUN apt-get install -y curl wget xz-utils git build-essential libz-dev zlib1g-dev libncurses5-dev ccache gawk unzip python python3 file
...
RUN git clone https://github.com/frainzy1477/luci-app-clash.git package/luci-app-clash
...
RUN sed -i 's/CONFIG_PACKAGE_luci-app-clash=m/CONFIG_PACKAGE_luci-app-clash=y/g' .config
...
RUN pushd package/luci-app-clash/tools/po2lmo && make && make install && popd
...
RUN make package/luci-app-clash/compile V=99
...
RUN mv `find /build/sdk/bin/packages/ | grep luci-app-clash` .
