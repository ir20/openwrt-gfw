...
RUN cp -r ../openwrt-passwall/shadowsocks-rust package/
...
RUN sed -i 's/CONFIG_PACKAGE_shadowsocks-rust=m/CONFIG_PACKAGE_shadowsocks-rust=y/g' .config
...
RUN make package/shadowsocks-rust/compile V=99
...
RUN mv `find /build/sdk/bin/packages/ | grep shadowsocks-rust` .
